...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
      ...
      document.addEventListener('click', (e) => {
        if (e.target.classList.contains('task-checkbox')) {
          ...
          if (action === 'Delete' && taskId) deleteTask(taskId);
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            loadTeacherCourses(user.email); // Load student's courses
        } else {
            currentUserId = null;
    ...
    // Change the function name from loadStudentCourses to loadTeacherCourses
async function loadTeacherCourses(userEmail) {
    if (!userEmail) {
        console.warn('No user email provided for course loading');
        return;
    }

    console.log('Loading courses for teacher:', userEmail);
    
    try {
        const courseRef = collection(db, 'courses');
        // Query courses where teacherId matches current user's ID
        const q = query(courseRef, where('teacherId', '==', auth.currentUser.uid));
        ...
        querySnapshot.forEach((doc) => {
            ...
            console.log('Found teacher course:', courseData.title);
            
            const option = document.createElement('option');
            option.value = courseData.title;
            // Show course code if available
            option.textContent = courseData.code ? 
        ...
        console.log(`Loaded ${coursesFound} teacher courses`);
...
function prepareAddTaskModal() {
    ...
    if (auth.currentUser) {
        loadTeacherCourses(auth.currentUser.email);
    }
